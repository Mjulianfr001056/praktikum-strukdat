...
int jumlahNode(ptrnode node, int cnt){
	if(!node){
		return 0;
	}
	
	cnt += jumlahNode(node->left, node->data) + jumlahNode(node->right, node->data);
	return cnt;
}

void getMinMax(ptrnode node){
	if(!node){
		return;
	}
