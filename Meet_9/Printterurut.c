...
int hitunglevel(struct node *root)
{
    if (root == NULL)
    {
        return 0;
    }
    int leftMaxLevel = 1 + hitunglevel(root->left);
    int rightMaxLevel = 1 + hitunglevel(root->right);
...
void print_terurut(struct node *root)
{
    int i;
    int levelCount = hitunglevel(root);
