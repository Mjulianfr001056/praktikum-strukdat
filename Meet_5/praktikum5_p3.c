...
ptrDLL DLL_reverse(ptrDLL currDLL);
...
int main(){
	...
	printf("Kondisi awal list: \n");
	printDLL(my_list, false);
	my_list = DLL_reverse(my_list);
...
ptrDLL DLL_reverse(ptrDLL currDLL){
	int i = 0, j = currDLL->capacity;
	
	ptrnode cursor = currDLL->tail;
	
	while(cursor){
		currDLL = DLL_pushBack(currDLL, cursor->data);
		cursor = cursor->prev;
	}
